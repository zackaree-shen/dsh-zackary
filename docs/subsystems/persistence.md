...
## Concurrent-writer refusal — a mutation that no longer continues the artifact

A backend whose artifact can be shared between processes refuses a durable mutation that no longer continues what its writer observed, with `SessionPersistenceConflictError` — distinct from `SessionPersistenceCorruptionError` (nothing is damaged) and from `SessionFormatUnsupportedError` (the log is readable). The refusal exists because a second writer's events already occupy the sequence numbers the stale writer's next batch claims: writing them would place two different events at one `seq`, which every reader must then refuse as a `seq` gap in the committed region.

The JSONL backend records an artifact's byte length and committed event count on every complete read and every durable write. An append is refused when either field no longer matches, and a truncate repair is refused when the byte length no longer matches, both before any byte changes. The event count is checked as well as the byte length because a detached read (`readFrom`) also observes the artifact: re-observing another writer's log must not silently re-arm a stale writer's base. A torn-frame repair re-records the truncated extent, so the records it recovered can be re-appended.

This detection is not coordination. It cannot make an append atomic against a writer that opens the same artifact in the same instant, so two processes deliberately writing one session still need a liveness signal outside the log — [postmortem 0005](../postmortem/0005-concurrent-process-writers-duplicated-committed-sequences.md) records the failure and the guardrails still outstanding.
