...
- **Concurrent-writer refusal.** Every complete read and every durable write records the artifact's byte length and committed event count. An append whose artifact no longer matches that extent in either field, or a truncate repair whose artifact no longer matches its byte length, is refused with `SessionPersistenceConflictError` before any byte changes: another process has advanced the log, and this writer's next events would repeat sequence numbers it already committed. A torn-frame repair re-records the truncated extent, so re-appending the records it recovered continues that extent.
...
- **One live writer per session** — append and repair are coordinated inside the owning backend instance, and a mutation the artifact no longer continues is refused with `SessionPersistenceConflictError` instead of being written (see [Concurrent-writer refusal](#durability-and-crash-semantics)). That refusal makes a second writer loud, not supported: it cannot make an append atomic against a writer that opens the same artifact in the same instant, so another instance or process must still not write the same session until its owner reaches quiescent disposal. Initial same-id publication remains collision-safe through the POSIX no-overwrite hard link or Windows write-through rename without replacement.
