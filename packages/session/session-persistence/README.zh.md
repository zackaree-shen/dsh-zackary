...
- **没有跨进程写入排他**：写入在单个进程内串行化，带守护的后端会拒绝不再延续其所观测范围的变更，因此第二个写入方会显式失败，而不会损坏日志。刻意并发访问同一会话仍需要日志之外的存活信号（[事故复盘 0005](../../../docs/postmortem/0005-concurrent-process-writers-duplicated-committed-sequences.md)）。
